...
nvcc -o convSharedMem convSharedMem.cu -std=c++11 --disable-warnings
...
do
	...
	totalTime=$( echo "$totalTime + $(./convSharedMem $search_dir$filename $out_dir$filename | awk '/\data/' | grep -Eo '[+-]?[0-9]+([.][0-9]+)?') " | bc )
